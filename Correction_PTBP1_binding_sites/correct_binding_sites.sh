...
bedtools intersect -s -a  ${path}/${complete_reads} -b  ${path}/${clusters} -wa >  ${path}/${complete_reads}-candidates.bed
...
sort -k1,1 -k2,2n -k6,6 ${path}/${complete_reads}-candidates-End.bed > ${path}/${complete_reads}-candidates-End-sorted.bed
mv ${path}/${complete_reads}-candidates-End-sorted.bed ${path}/${complete_reads}-candidates-End.bed
bedtools closest -s -D a -io -iu -t first -a ${path}/${clusters} -b ${path}/${complete_reads}-candidates-End.bed > ${path}/${clusters}-closest-iCLIP-end.BED
...
bedtools intersect -s -a ${path}/${incomplete_reads} -b ${path}/${clusters} -wa > ${path}/${incomplete_reads}-candidates.bed
python getEnd-BED.py ${path}/${incomplete_reads}-candidates.bed ${path}/${incomplete_reads}-candidates-End.bed
sort -k1,1 -k2,2n -k6,6 ${path}/${incomplete_reads}-candidates-End.bed > ${path}/${incomplete_reads}-candidates-End-sorted.bed
mv ${path}/${incomplete_reads}-candidates-End-sorted.bed ${path}/${incomplete_reads}-candidates-End.bed
...
rm ${path}/${clusters}-closest-iCLIP-end-corrected.bed-NOT.bed-closest.bed
